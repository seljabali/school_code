...
void R_xmlFreeDoc(SEXP ref)
{
  ...
  if(doc) {
      const xmlChar *url = doc->URL ? doc->URL : (doc->name ? doc->name : "?? (internally created)");
#ifdef R_XML_DEBUG
      fprintf(stderr, "Cleaning up document %p, %s\n", (void *) doc, url);
#endif
      xmlFreeDoc(doc);
      R_numXMLDocsFreed++;
  }
...
RS_XML_freeDoc(SEXP ref)
{
  ...
  if(doc) {
      const xmlChar *url = doc->URL ? doc->URL : (doc->name ? doc->name : "?? (internally created)");
#ifdef R_XML_DEBUG
      fprintf(stderr, "Freeing just the document %p, %s\n", (void *) doc, url);
#endif
      xmlFreeDoc(doc);
      R_numXMLDocsFreed++;
  }

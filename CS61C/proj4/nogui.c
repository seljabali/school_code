...
void display_cache()
{
  ...
  {
  case INDEX:
    printf("Set V D  LRU\tTag\n=== = =  ===\t===\n");
    for(b = 0; b < set_count; b++)
    {
      for(s = 0; s < assoc; s++)
      {
	printf("%2d  %d %d  %s\t%08x    ", b, cache[b].block[s].valid, cache[b].block[s].dirty, lru_to_string(b, s), cache[b].block[s].tag);
...
{
  printf("\n");
  printf("TIPS is a MIPS R2000 simulator with a cache module written by a CS61c student\n");
  ...
  printf("  Policy> is either 'l' for LRU or 'r' for RANDOM. <Sync Policy> is either\n");
  printf("  'wb' for WRITE_BACK or 'wt' for WRITE_THROUGH\n");
  printf("\n");

...
void configure_cache_drawing_parameters(GtkWidget* widget)
{
  ...
  case ASSOC:
    ...
    break;
  case INDEX:
    cache_header_text = "  Set V D LRU\tTag\n";
    break;
  default:
  ...
  block_header_text = "  %2d  %d %d %s\t%08X   ";
  buffer_size = sprintf(buffer, block_header_text, 0, cache[0].block[0].valid, cache[0].block[0].dirty, lru_to_string(0, 0), cache[0].block[0].tag);
  pango_layout_set_text(layout, buffer, buffer_size);
...
gboolean draw_cache_display_index(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
  ...
  if(assoc == 0 || set_count == 0 || block_size == 0)
  {
    ...
    buffer_size = sprintf(buffer, "\n\n\nNo cache exists - Click on 'Config Cache' to configure a cache");
    pango_layout_set_text(layout, buffer, buffer_size);
    gdk_draw_layout(widget->window, 
		    widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
		    base_x_offset, 
  ...
  for(b = 0; b < set_count; b++)
  {
    for(s = 0; s < assoc; s++)
    {
      buffer_size = sprintf(buffer, block_header_text, b, cache[b].block[s].valid, cache[b].block[s].dirty, lru_to_string(b, s), cache[b].block[s].tag);
      pango_layout_set_text(layout, buffer, buffer_size);
      gdk_draw_layout(widget->window, 
		      widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
		      x_offset,
...
gboolean draw_cache_display_assoc (GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
  ...
  for(s = 0; s < assoc; s++)
  {
    ...
    for(b = 0; b < set_count; b++)
    {      
      buffer_size = sprintf(buffer, block_header_text, s, cache[b].block[s].valid, cache[b].block[s].dirty, lru_to_string(b, s), cache[b].block[s].tag);
      pango_layout_set_text(layout, buffer, buffer_size);
      gdk_draw_layout(widget->window, 
		      widget->style->fg_gc[GTK_WIDGET_STATE(widget)],
		      x_offset,
...
gboolean configure_button_listener(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
  ...
  case GTK_RESPONSE_ACCEPT:
    ...
    assert(panel_replacement_policy == RANDOM || panel_replacement_policy == LRU);
    ...
    sprintf(buffer, "Cache parameters changed:\n + set count = %d\n + associativity = %d\n + block size = %d\n + replacement policy = %s\n + memory sync policy = %s\n", set_count, assoc, block_size, (policy == RANDOM ? "Random" : "LRU"), (memory_sync_policy == WRITE_BACK ? "Write Back" : "Write Through"));
    append_log(buffer);
  ...
  case GTK_RESPONSE_REJECT:
    /* printf("do not update cache"); !!! fill in responses */
    break;
  default:
...
gboolean load_button_listener(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
  ...
  /* Better dialog code for updated GTK API
  ...
  */
  ...
  dialog = gtk_file_selection_new("Load program");
#ifdef CYGWIN
  gtk_file_selection_hide_fileop_buttons(GTK_FILE_SELECTION(dialog));
#endif
  /* Ensure that the dialog box will load the file if one is selected */
...
GtkWidget* build_button_panel()
{
  ...
  gtk_tooltips_set_tip(GTK_TOOLTIPS(button_panel_tooltips), reset_cache_button, "Flushs the contents of the cache", NULL);
  ...
  gtk_table_attach_defaults(GTK_TABLE(table1), load_button, 0, 1, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(table1), step_button, 1, 2, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(table1), run_button, 2, 3, 0, 1);

  table2 = gtk_table_new(1, 3, TRUE);
  gtk_table_attach_defaults(GTK_TABLE(table2), reset_machine_button, 0, 1, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(table2), reset_cache_button, 1, 2, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(table2), reset_output_button, 2, 3, 0, 1);
  ...
  gtk_container_add(GTK_CONTAINER(op_frame), table1);

  reset_frame = gtk_frame_new("Reset");
  gtk_container_add(GTK_CONTAINER(reset_frame), table2);

  quit_frame = gtk_frame_new("Quit");
  gtk_container_add(GTK_CONTAINER(quit_frame), quit_button);

  panel = gtk_table_new(1, 7, TRUE);
  gtk_table_attach_defaults(GTK_TABLE(panel), configure_frame, 0, 1, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(panel), op_frame, 1, 4, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(panel), reset_frame, 4, 7, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(panel), quit_frame, 7, 8, 0, 1);
...
int build_gui(int argc, char** argv)
{
  ...
  /*  Build layout */  
  border_layout = gtk_table_new(100, 100, TRUE);
  gtk_container_add(GTK_CONTAINER(main_window), border_layout);
  ...
  gtk_widget_set_size_request(button_panel, WINDOW_DEFAULT_WIDTH, WINDOW_DEFAULT_HEIGHT * .07);

  /* Place panels into window */  
  gtk_table_attach(GTK_TABLE(border_layout), register_panel, 0, 50, 0, 25, 0, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(border_layout), log_panel, 50, 100, 0, 25, 0, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(border_layout), drawing_panel, 0, 100, 25, 93, 0, 0, 0, 0);
  gtk_table_attach(GTK_TABLE(border_layout), button_panel, 0, 100, 93, 100, 0, 0, 0, 0);\

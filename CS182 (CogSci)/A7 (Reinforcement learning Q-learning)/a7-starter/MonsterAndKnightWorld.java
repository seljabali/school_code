...
public class MonsterAndKnightWorld implements RLWorld {
    public int bx, by;
    ...
    public boolean gotTreasure = false;

    public int monsterscore = 0, knightscore = 0;

    public int treasureReward = 50, deathPenalty = 100;
    ...
    public MonsterAndKnightWorld(int x, int y, int numWalls) {
        bx = x;
        by = y;
    ...
    public MonsterAndKnightWorld(int x, int y, boolean[][] newwalls) {
        bx = x;
        by = y;
    ...
    public int[] getNextState(int action) {
        // action is knight action: 0=u 1=ur 2=r 3=dr ... 7=ul
        ...
        // if knight has treasure, relocate treasure
        if ((mx == chx) && (my == chy)) {
            d = getRandomPos();
            chx = d.width;
            chy = d.height;
         ...
         // if monster has knight, relocate knight 
        //if ((mx==cx) && (my==cy)) {
        //	d = getRandomPos(); mx = d.width; my = d.height; 
        //}
    ...
    public double calcReward() {
        ...
        // if ((mx==hx)&&(my==hy)&&(gotTreasure)) newReward += 100;
        return newReward;
    }
    ...
    boolean endGame() {
        // return (((mx==hx)&&(my==hy)&& gotTreasure) || ((cx==mx) &&
        // (cy==my)));
        return ((cx == mx) && (cy == my));
    }
    ...
    void makeWalls(int xdim, int ydim, int numWalls) {
        ...
        for (int t = 0; t < WALL_TRIALS; t++) {
            ...
            for (int i = 0; i < numWalls; i++) {
                ...
                double dropperc = Math.sqrt((dx2 + dy2)
                        / (xmid * xmid + ymid * ymid));

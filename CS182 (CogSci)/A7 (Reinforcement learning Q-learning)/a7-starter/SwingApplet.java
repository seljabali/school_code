...
public class SwingApplet extends JApplet implements ActionListener, Runnable {
    ...
    MonsterAndKnightGame game;

    MonsterAndKnightWorld trainWorld, playWorld; // seperate world from
                                                    // playing world
    ...
    final String INSTRUCT_MESSAGE = "<html><p>This applet demonstrates how reinforcement <p>learning can be used to train an agent to play <p>a simple game.  In this case the game is Monster and <p>Knight- the knight tries to get to the treasure <p>and back to his home, the monster tries to catch the knight.",
            USAGE_MESSAGE = "<html><p>You can train the agent by selecting the Train tab.  At <p>any time you can select the Play tab to see how <p>well the agent is performing!  Of course, the more <p>training, the better the chance the knight <p>has of surviving :)";
    ...
    public int knightscore = 0, monsterscore = 0;

    JLabel monsterscorelabel, knightscorelabel;

    final String MS_TEXT = "Knight Score:", CS_TEXT = "Monster Score:";

    JSlider speed, smoothSlider;

    Image monsterImg, knightImg;

    chartPanel graphPanel;

    JLabel winPerc;

    boardObject monster, knight, treasure, back, hole, wall;
    ...
    public void init() {
        // load images
        monsterImg = getImage(getCodeBase(), "monster.gif");
        knightImg = getImage(getCodeBase(), "knight.gif");
        Image wallImg = getImage(getCodeBase(), "wall.gif");
        Image treasureImg = getImage(getCodeBase(), "treasure.gif");
        Image floorImg = getImage(getCodeBase(), "floor.gif");

        /*
         * Image monsterImg =
         * getImage(ClassLoader.getSystemResource("monster.gif")); Image
         * knightImg = getImage(ClassLoader.getSystemResource("knight.gif"));
         * Image wallImg = getImage(ClassLoader.getSystemResource("wall.gif"));
         * Image treasureImg =
         * getImage(ClassLoader.getSystemResource("treasure.gif"));
         */

        // set up board objects
        monster = new boardObject(monsterImg);
        knight = new boardObject(knightImg);
        treasure = new boardObject(treasureImg);
    ...
    public void worldInit(int xdim, int ydim, int numwalls) {
        trainWorld = new MonsterAndKnightWorld(xdim, ydim, numwalls);
        gameInit(xdim, ydim);
    }

    public void worldInit(boolean[][] givenWalls) {
        int xdim = givenWalls.length, ydim = givenWalls[0].length;
        trainWorld = new MonsterAndKnightWorld(xdim, ydim, givenWalls);
        gameInit(xdim, ydim);
    }
    ...
    private void gameInit(int xdim, int ydim) {
        ...
        playWorld = new MonsterAndKnightWorld(xdim, ydim, trainWorld.walls);
        ...
        game = new MonsterAndKnightGame(this, DELAY, playWorld, rl.getPolicy());
        game.start();

        // set text fields on panels
        penalty.setText(Integer.toString(trainWorld.deathPenalty));
        reward.setText(Integer.toString(trainWorld.treasureReward));
    ...
    public void updateBoard() {
        // update score panels
        knightscorelabel.setText(MS_TEXT + " " + Integer.toString(knightscore));
        monsterscorelabel.setText(CS_TEXT + " "
                + Integer.toString(monsterscore));
        if (game.newInfo) {
            updateScore();
            game.newInfo = false;
        }
        ...
        // draw objects (monster over knight over treasure)
        bp.setSquare(treasure, game.getTreasure());
        bp.setSquare(knight, game.getKnight());
        bp.setSquare(monster, game.getMonster());
    ...
    void doTraining() {
        ...
        // start training
        trainWorld.treasureReward = cval;
    ...
    void updateScore() {
        double newScore = Math.round(1000 * ((double) knightscore)
                / (monsterscore + knightscore)) / 10;
        ...
        startbutt.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (sampleWorld) {
                    ...
                    // random world
                    worldInit(Integer.parseInt(cols.getText()), Integer
                            .parseInt(rows.getText()), Integer.parseInt(obst
                            .getText()));
                }
            }
        });
    ...
    Container customWorld() {
        ...
        // labelpane.setBorder(BorderFactory.createTitledBorder("Custom
        // World"));
    ...
    Container chooseWorld() {
        ...
        for (int i = 0; i < samples.numSamples(); i++) {
            ...
            boardPanel pic = new boardPanel(back, w.length, w[0].length,
                    SAMP_W, SAMP_H);
            // add walls to panel
    ...
    Container makeParamPanel() {
        ...
        labelpane.add(new JLabel("Treasure Reward:", JLabel.RIGHT));
    ...
    Container playControlPanel() {
        ...
        // add controls to select greedy or rl knight
        // JPanel learnButtons = new JPanel();
        // learnButtons.setLayout(new GridLayout(1,0));
        JRadioButton greedy = new JRadioButton("Greedy Knight");
        JRadioButton smart = new JRadioButton("Smart Knight", true);
        greedy.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                // set game to use greedy knight
                game.knighttype = MonsterAndKnightGame.GREEDY;
            }
        });
        smart.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                // set game to use smart knight
                game.knighttype = MonsterAndKnightGame.SMART;
            }
        });
        ButtonGroup knightButts = new ButtonGroup();
        knightButts.add(greedy);
        knightButts.add(smart);
    ...
    Container scorePanel() {
        ...
        // score labels
        ImageIcon monster = new ImageIcon(monsterImg);
        ImageIcon knight = new ImageIcon(knightImg);
        knightscorelabel = new JLabel(MS_TEXT, knight, JLabel.RIGHT);
        monsterscorelabel = new JLabel(CS_TEXT, monster, JLabel.RIGHT);
        ...
        scorePane.add(knightscorelabel);
        scorePane.add(winPerc);
        scorePane.add(monsterscorelabel);
        scorePane.add(reset);
...
class chartPanel extends JPanel {
    ...
    public void updateScores() {
        int m = a.knightscore, c = a.monsterscore;
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int x = 0; x < history.size(); x++) {
            ...
            // thisval="+thisval+"newval="+newval+" xval="+xval+" yval="+yval+"
            // previous="+previous);
    ...
    void addScore(double s) {
        ...
        /*
         * System.out.println("History being
         * pruned."+Thread.currentThread().getName()); Vector nVec = new
         * Vector(); for (int i=(MAXSIZE/3); i<history.size(); i++) {
         * nVec.addElement(history.elementAt(i)); } history = nVec;

...
vector_t *vector_new() {
	 ...
	 * instead of 'struct _vector_t *'?  Does it matter?  Its been typdef'd in vector.h
	 */
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    {
       data_temp = (int *)malloc((loc+1) * sizeof(int));
       
       for(i=0; i < (loc+1); i++) /*Initialize temp*/
	   data_temp[i] = 0;
	       
       for (i = 0; i < (v->size); i++) /*Copy old data to new array*/
       {

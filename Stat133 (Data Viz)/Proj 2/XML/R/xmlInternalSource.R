...
#  library(XML)  
  ...
  if(length(xpath)) {
    ...
    if(length(section))  {
       functions = paste("//section[", section, "]", functions, sep = "")
       xnodes = paste("//section[", section, "]", xnodes, sep = "")
    }
...
#XXX
  ...
  ans = sapply(url, evalNode, envir = envir, verbose = verbose, ids = ids, omit = omit, echo = echo, print = print)
  invisible(ans)
})
...
function(node, envir = globalenv(), ids = character(), verbose = FALSE, echo = echo, omit = character(),
         namespaces = c(r = "http://www.r-project.org"), print = echo)
{

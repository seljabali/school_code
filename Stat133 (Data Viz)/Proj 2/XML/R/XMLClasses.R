          ...
          resolveNamespaces = TRUE)
{
  ...
  if(is.null(tmp)) {
    if(FALSE) {
       doc = newXMLDoc(node = node, addFinalizer = FALSE) # , addFinalizer = getNativeSymbolInfo("R_xmlFreeDocLeaveChildren")$address)
    } else {
...
#    doc = newXMLDoc(node = node, addFinalizer =getNativeSymbolInfo("R_xmlFreeDocLeaveChildren")$address)
  ...
  w = sapply(ans, function(el) .Call("RS_XML_isDescendantOf", el, node, TRUE))

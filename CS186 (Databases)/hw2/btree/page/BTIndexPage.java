...
public class BTIndexPage extends BTSortedPage
{
	...
	/*
	 * It is used in full delete @param indexPage the sibling page of this.
	 * Input parameter. @param parentIndexPage the parant of indexPage and this.
	 * Input parameter. @param direction -1 if "this" is left sibling of
	 * indexPage ; 1 if "this" is right sibling of indexPage. Input parameter.
	 * @param deletedKey the key which was already deleted, and cause
	 * redistribution. Input parameter. @exception RedistributeException
	 * Redistribution failed @return true if redistrbution success. false if we
	 * can not redistribute them.
	 */
	public boolean redistribute(BTIndexPage indexPage, BTIndexPage parentIndexPage,
			int direction, Key deletedKey) throws RedistributeException
	{
		...
		try
		{
			...
			{ // 'this' is the left sibling of indexPage
				if ((getSlotLength(getSlotCnt() - 1) + available_space()) > ((PAGE_SIZE - DPFIXED) / 2))
				{
				...
				} else
				{
					...
					indexPage.setLeftLink((PageId)lastEntry.getData());
					...
					if (deleteSortedRecord(delRid) == false)
						throw new RedistributeException(null,
			...
			{ // 'this' is the right sibling of indexPage
				if ((getSlotLength(0) + available_space()) > ((PAGE_SIZE - DPFIXED) / 2))
				{
					// cannot spare a record for its underflow sibling
					return false;
				} else
				{
					...
					setLeftLink((PageId)firstEntry.getData());
